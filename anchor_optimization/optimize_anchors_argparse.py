...
def parse_args(args):
    ...
    parser.add_argument('--mutation', type=float,
                        ...
                        default=0.5)
    parser.add_argument('--threads', type=int, help='The number of threads to run in parallel. If > 1 then "updating" is set to "deferred" by default', default=1)
    parser.add_argument('--verbose', action='store_true', help='Should information be printed out in the console.')
...
def main(args=None):
    ...
    res_argp = anchors_optim(annotations = args.annotations,
                             ...
                             SIZES = [32, 64, 128, 256, 512],                            # default SIZES values based on keras-retinanet
                             STRIDES = [8, 16, 32, 64, 128],                             # default STRIDES values based on keras-retinanet
